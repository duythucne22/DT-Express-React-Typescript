...
import { generateDetouredRoute } from '../../utils/routeGeometry';
...
export class CheapestStrategy implements RouteStrategy {
  ...
  calculate(input: RoutingApiRequest): RoutingStrategyResult {
    ...
    const distanceKm = Number((directDistance * 1.15).toFixed(1));
    const avgSpeedKmH = 56;
    const hours = distanceKm / avgSpeedKmH;

    // Generate route with significant detours (cheapest = uses secondary roads/consolidation)
    const geometry = generateDetouredRoute(
      { lat: input.origin.latitude, lng: input.origin.longitude },
      { lat: input.destination.latitude, lng: input.destination.longitude },
      0.85, // High detour factor for cheapest route
      20    // 20 waypoints for smooth, complex path
    );

    return {
      strategyUsed: this.name,
      waypointNodeIds: ['NODE-ORIGIN', 'NODE-TRANSFER', 'NODE-ECONOMY', 'NODE-DEST'],
      distanceKm,
      estimatedDuration: toDuration(hours),
      estimatedCost: {
        amount: Number((distanceKm * 0.028 + input.packageWeight.value * 1.5).toFixed(2)),
        currency: 'CNY',
      },
      carrierCode: 'JD',
